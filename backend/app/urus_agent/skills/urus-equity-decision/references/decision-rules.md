...
- `technical.rsi_context` distinguishes high-momentum continuation from exhaustion and oversold continuation from reversal. Treat its classification and scores as deterministic supporting evidence, not as an order or standalone action.
- An overbought reading is not an automatic exit, and an oversold reading is not an automatic entry. Confirm RSI context with price structure, relative strength, market/theme regime, volume, event risk, and data quality.
- `breakout_confirmed` and `reversal_confirmed` may strengthen an otherwise supported setup; `exhaustion_watch` and `breakdown_confirmed` may strengthen a risk or invalidation case. None of these labels can alone justify `setup_ready`, `avoid`, or an if-held exit.
