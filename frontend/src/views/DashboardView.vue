  ...
  return [
    {
      id: 'market',
      label: '大盘 / 1A',
      meta: snapshot ? `${snapshot.returned_symbols.length}/${snapshot.requested_symbols.length} 个快照` : '未采集',
...
<template>
  ...
  <main class="page-shell validation-page">
    <header class="validation-header">
      <div>
        <p class="eyebrow">STAGE 1A / DATA VALIDATION</p>
        <h1>数据采集验证</h1>
      </div>
    ...
    <section v-if="store.latestRun && store.latestReadModel" class="validation-workspace">
      ...
      <section v-else-if="activeTab === 'options'" class="tab-panel" role="tabpanel">
        <div class="tab-titlebar"><div><p class="eyebrow">NOT CONNECTED / 2</p><h2>期权数据</h2></div><StatusBadge :status="store.latestReadModel.options.data_state" /></div>
        <section class="data-section"><div class="metric-grid primary-metrics"><div class="metric-cell"><span>状态</span><strong>{{ store.latestReadModel.options.status }}</strong><small>执行状态：占位</small></div><div class="metric-cell"><span>可用</span><strong>{{ store.latestReadModel.options.available ? '是' : '否' }}</strong><small>未采集期权链</small></div></div><div class="notice-box"><strong>当前记录</strong><span>{{ store.latestReadModel.options.note }}</span></div></section>
        <section class="data-section unfinished-section"><div class="section-label-row"><div><span class="section-kicker">NOT COLLECTED</span><h3>未接入字段</h3></div></div><div class="unfinished-list"><span>IV 曲面</span><span>Skew / 期限结构</span><span>隐含波动区间</span><span>GEX</span><span>关键到期日</span><span>0DTE 链</span></div></section>
      </section>
