    ...
    <section class="trace-layout">
      ...
      <aside class="trace-inspector">
        ...
        <template v-else-if="selectedNode">
          ...
          <div v-if="selectedNode.decision_run" class="trace-run-meta"><span>provider {{ selectedNode.decision_run.provider }}</span><span>model {{ selectedNode.decision_run.model || '—' }}</span><span>tools {{ selectedNode.decision_run.tool_call_count }}</span><span>temperature {{ selectedNode.decision_run.temperature ?? '—' }}</span><span>tokens {{ selectedNode.decision_run.prompt_tokens ?? 0 }}/{{ selectedNode.decision_run.completion_tokens ?? 0 }}</span><span v-if="selectedNode.decision_run.estimated_cost != null">cost {{ selectedNode.decision_run.estimated_cost }}</span></div>
